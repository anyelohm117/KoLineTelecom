...
// Si intenta entrar alguien que no es Admin (Soporte o Cliente)
if ($_SESSION['rol'] != 1) {
    echo "<script>
            alert('⛔ ACCESO DENEGADO: Solo el Administrador puede acceder a la configuración del sistema.');
            window.location.href='dashboard.php';
          </script>";
    exit();
}

// Variable para controlar el menú (siempre true aquí)
$es_admin = true;
...
<link rel="icon" type="image/png" href="imagenes/logo.png?v=2">
...
.config-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 25px; }
.card-panel { background: var(--glass-bg); backdrop-filter: blur(12px); padding: 25px; border-radius: 20px; border: 1px solid var(--glass-border); }
.card-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 15px; margin-bottom: 20px; }
.card-header h3 { margin: 0; color: var(--accent); font-weight: 500; }

/* Lista de Planes */
.plan-item { display: flex; justify-content: space-between; align-items: center; background: rgba(255,255,255,0.03); padding: 15px; border-radius: 10px; margin-bottom: 10px; border: 1px solid transparent; transition: 0.3s; }
...
.btn-delete { color: #ff3355; text-decoration: none; font-size: 18px; padding: 5px; transition: 0.3s; }
...
.mini-form input { background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1); padding: 10px; border-radius: 6px; color: white; width: 100%; outline: none; }
.mini-form input:focus { border-color: var(--accent); }
.btn-mini { background: var(--accent); color: var(--bg-dark); border: none; padding: 0 20px; border-radius: 6px; font-weight: bold; cursor: pointer; transition: 0.3s; }
.btn-mini:hover { background: var(--accent-hover); box-shadow: 0 0 10px var(--accent); }
